...
public class Parser
{
    ...
    public int[] findConversionRange(int pos)
    {
        ...
        if (pos >= 0 && pos < script.size())
        {
            ...
            {
                if (stop < script.size() && !Check.isConvertable(script.get(stop).getComponent()))
                    break;
                if (stop < script.size() - 1 && !Check.isConvertable(script.get(stop + 1).getComponent()))
                    break;
                stop++;
            }
    ...
    private void elementListToComponentList()
    {
        ...
        {
            if (e.getComponent() == Component.ANSWER || e.getComponent() == Component.ANSWER_LAST)
                answerIds.add(e.getAnswerId());
